...
utilisateur dans Supabase Auth avec l'email `christian@pizzamanstjean.fr`, puis
définir son mot de passe dans Supabase. Sur `pizzeria.html`, l'identifiant
`christian` est automatiquement relié à cet email.

Les clients peuvent créer des commandes sans compte. La lecture, la modification
et la suppression sont réservées au compte authentifié
`christian@pizzamanstjean.fr`.
